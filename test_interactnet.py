...
parser = OptionParser()
#Set configurable options to be entered at command line.
...
#Get file location of config options.
config_output_filename = options.config_filename

#Load in class count, class mapping, action count, and 
#action mappings via their respective pickle files.
...
#Open C (which holds model configuration settings) via 
#its pickle file
...
#Construct background class if not included.
if 'bg' not in class_mapping:
    class_mapping['bg'] = len(class_mapping)

#Get inverse class mapping.
class_mapping = {v: k for k, v in class_mapping.items()}
print(class_mapping)
#Set each class to a random color
class_to_color = {class_mapping[v]: np.random.randint(0, 255, 3) for v in class_mapping}
#Get number of ROIs to process.
C.num_rois = int(options.num_rois)

#Determine number of features based on base network.
if C.network == 'resnet50':
    num_features = 1024
elif C.network == 'xception':
    num_features = 1024
...
#Set image dimension ordering based on backend.
...
#Define input layers for images, ROIs, and feature maps.
img_input = Input(shape=input_shape_img)
roi_input = Input(shape=(C.num_rois, 4))
feature_map_input = Input(shape=input_shape_features)
human_rois = Input(shape=(C.num_rois, 4))
...
#Define the classifier layer (first branch)
...
#For each test image in the specified directory...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
    #only process files with the proper file type
    if not img_name.lower().endswith(('.bmp', '.jpeg', '.jpg', '.png', '.tif', '.tiff')):
        continue
    print(img_name)
    st = time.time()
    #Get filepath of each image
    filepath = os.path.join(img_path,img_name)
    #Read in each image.
    img = cv2.imread(filepath)
    #Format image into processable format
    X, ratio = format_img(img, C)
    #Rearrange dimensions according to tf specification.
	...
	#Take a slice of R of width C.num_rois
        ROIs = np.expand_dims(R[C.num_rois*jk:C.num_rois*(jk+1), :], axis=0)
        if ROIs.shape[1] == 0:
            break
	#If on the last iteration, and slice may not be filled...
    ...
    #Run second branch
        ...
        for ii in range(P_cls.shape[1]):
            ...
            if np.max(P_cls[0, ii, :]) < bbox_threshold or np.argmax(P_cls[0, ii, :]) == (P_cls.shape[2] - 1):
                continue
            #Get class name for current iteration
